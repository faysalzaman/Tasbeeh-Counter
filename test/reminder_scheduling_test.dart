...
void main() {
  group('Default Dhikrs Reminder Configuration', () {
    test('every default dhikr has reminderEnabled = true and valid reminderTime', () {
      ...
      for (final map in AppConstants.defaultDhikrs) {
        ...
        expect(
          ...
          isTrue,
          reason: 'Dhikr "$name" ($id) has invalid time format "$time". Expected HH:mm',
        );
      }
    });
    ...
    test('DhikrRepository effective reminder resolution works for default and overridden dhikrs', () {
      final sampleMap = AppConstants.defaultDhikrs.firstWhere((d) => d['id'] == 'durood_friday_80');
      ...
      expect(repo.getEffectiveReminderTime(dhikr, incrementProgress), equals('16:30'));
      ...
      expect(repo.getEffectiveReminderTime(dhikr, changedTimeProgress), equals('18:00'));
    });
  });
}
